...
void UInventoryComponent::TryAddFromLootTable(FLootTableRoll LootTable, TArray<FItemAddResult>& OutItemAddResults)
{
...
FItemAddResult UInventoryComponent::TryAddItemFromClass(TSubclassOf<class UItem> ItemClass, const int32 Quantity /*=1*/, const bool bCheckAutoUse/*=false*/)
{
...
bool UInventoryComponent::HasItem(TSoftClassPtr<class UItem> ItemClass, const int32 Quantity /*= 1*/, const bool bCheckVisibility/*= false*/) const
{
	if (!ItemClass.IsNull())
	{	
...
bool UInventoryComponent::AllowLootItem(class UInventoryComponent* Taker, TSubclassOf<class UItem> ItemClass, const int32 Quantity, FText& ErrorText)  const
{
	...
	if (Taker)
	{
		...
		const bool bShopHasItem = HasItem(TSoftClassPtr<UItem>(ItemClass), Quantity);
...
bool UInventoryComponent::AllowStoreItem(class UInventoryComponent* Storer, TSubclassOf <class UItem> ItemClass, const int32 Quantity, FText& ErrorText) const
{
	if (Storer)
	{
		const bool bWeHaveSpace = GetSpaceForItem(ItemClass, ErrorText) >= Quantity;
		const bool bStorerHasItem = Storer->HasItem(TSoftClassPtr<UItem>(ItemClass), Quantity);
...
FItemAddResult UInventoryComponent::PerformLootItem(class UInventoryComponent* Taker, TSubclassOf <class UItem> ItemClass, const int32 Quantity /*= 1*/)
{
	if (Taker)
	{
		...
		if (AddResult.AmountGiven > 0)
		{
			if (UItem* Item = FindItemByClass(TSoftClassPtr<UItem>(ItemClass)))
			{
...
FItemAddResult UInventoryComponent::PerformStoreItem(class UInventoryComponent* Storer, TSubclassOf <class UItem> ItemClass, const int32 Quantity /*= 1*/)
{
	if (Storer)
	{
		...
		if (AddResult.AmountGiven > 0)
		{
			if (UItem* Item = Storer->FindItemByClass(TSoftClassPtr<UItem>(ItemClass)))
			{
...
UItem* UInventoryComponent::FindItemByClass(TSoftClassPtr<class UItem> ItemClass, const bool bCheckVisibility/*= false*/) const
{
...
TArray<UItem*> UInventoryComponent::FindItemsOfClass(TSubclassOf<class UItem> ItemClass, const bool bCheckVisibility/*= false*/) const
{
...
int32 UInventoryComponent::GetSpaceForItem(TSubclassOf<class UItem> ItemClass, FText& NoSpaceReason) const 
{
	...
	if (const UItem* ItemCDO = GetDefault<UItem>(ItemClass))
	{
		...
		if (UItem* ExistingItem = FindItemByClass(TSoftClassPtr<UItem>(ItemClass)))
		{
...
void UInventoryComponent::SetInventoryFriendlyName(const FText& Name)
{
	InventoryFriendlyName = Name;
}

void UInventoryComponent::SetIsVendor(const bool bNewIsVendor)
{
...
//bool UInventoryComponent::Save(const FString& SaveName /*= "NarrativeInventorySaveData"*/, const int32 Slot /*= 0*/)
/*{
...
//bool UInventoryComponent::Load(const FString& SaveName /*= "InventorySaveData"*///, const int32 Slot /*= 0*/)
/*{
	...
	if (UInventorySaveGame* InventorySave = Cast<UInventorySaveGame>(UGameplayStatics::LoadGameFromSlot(SaveName, Slot)))
	{
		...
		for (auto& InventoryItem : InventorySave->SavedItems)
		{
			if (IsValid(InventoryItem.ItemClass) && InventoryItem.Quantity > 0)
			{
				...
				if (UItem* AddedItem = AddResult.Stacks[0])
				{
...
//bool UInventoryComponent::DeleteSave(const FString& SaveName /*= "InventorySaveData"*///, const int32 Slot /*= 0*/)
/*{
...
void UInventoryComponent::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION(UInventoryComponent, LootSource, COND_OwnerOnly);
...
UItem* UInventoryComponent::AddItem(TSubclassOf<class UItem> ItemClass, const int32 Quantity)
{
...
void UInventoryComponent::OnRep_Items(const TArray<class UItem*>& OldItems)
{
	OnInventoryUpdated.Broadcast();
...
void UInventoryComponent::OnRep_LootSource(class UInventoryComponent* OldLootSource)
{
	if (LootSource)
	{
...
FItemAddResult UInventoryComponent::TryAddItem_Internal(TSubclassOf<class UItem> ItemClass, const int32 Quantity /*= 1*/)
{
...
void UInventoryComponent::GiveDefaultItems()
{
...
void UInventoryComponent::ServerRemoveItem_Implementation(class UItem* Item)
{
	if (Item)
	{
		RemoveItem(Item);
...
void UInventoryComponent::ServerConsumeItem_Implementation(class UItem* Item, const int32 Quantity)
{
	if (Item)
	{
...
void UInventoryComponent::ServerUseItem_Implementation(class UItem* Item)
{
	UseItem(Item);
}
...
bool UInventoryComponent::RequestLootItem(class UItem* ItemToLoot, FText& ErrorText, const int32 Quantity /*= 1*/)
{
...
void UInventoryComponent::ServerRequestLootItem_Implementation(class UItem* ItemToLoot, const int32 Quantity)
{
	FText DummyText;
	RequestLootItem(ItemToLoot, DummyText, Quantity);
}

bool UInventoryComponent::RequestStoreItem(class UItem* ItemToStore, FText& ErrorText, const int32 Quantity /*= 1*/)
{
...
void UInventoryComponent::ServerRequestStoreItem_Implementation(class UItem* ItemToLoot, const int32 Quantity)
{
	FText DummyText;
	RequestStoreItem(ItemToLoot, DummyText, Quantity);
}

int32 UInventoryComponent::GetBuyPrice_Implementation(TSubclassOf<class UItem> Item, int32 Quantity /*= 1*/) const
{
	if (IsValid(Item))
	{
...
int32 UInventoryComponent::GetSellPrice_Implementation(TSubclassOf<class UItem> Item, int32 Quantity /*= 1*/) const
{
	if (IsValid(Item))
	{
		if (const UItem* ItemCDO = GetDefault<UItem>(Item))
...
void UInventoryComponent::SetLootSource(class UInventoryComponent* NewLootSource)
{
